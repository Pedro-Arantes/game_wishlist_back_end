...
export async function updatePictUser(req: AuthenticatedRequest, res: Response) {
  const { pictureId } = req.params;
  const { userId } = req;
  try {
    const result = await updatePicture(Number(pictureId),userId)
